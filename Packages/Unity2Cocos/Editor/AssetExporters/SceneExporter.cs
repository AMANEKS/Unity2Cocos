...
namespace Unity2Cocos
{
	...
	public class SceneExporter : AssetExporter<UnityEditor.SceneAsset>
	{
		...
		public override string Export(UnityEditor.SceneAsset asset)
		{
			...
			if (sunSource)
			{
				// Rotate the default skybox art to roughly match the sun light direction.
			...
			if (RenderSettings.skybox)
			{
				...
				if (!string.IsNullOrEmpty(envmapUuid))
				{
					...
					// The exported skybox is already oriented as authored in Unity.
					skyBoxInfo._rotationAngle = 0;
				}
			}
